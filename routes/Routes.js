...
const { listBuku, cariBuku, tambahBuku, hapusBuku, rubahStatusPeminjaman } = require('../repos/LibraryRepo');

// TODO: Memanggil fungsi listBuku untuk mendapatkan data semua buku yang ada
app.get('/', (req, res) => {
  const { id } = req.params;
  repo.listBuku(id).then((buku) => {
    res.json(buku);
...
app.put('/:id', (req, res) => {
  const { id } = req.params;
  repo.rubahStatusPeminjaman(id).then(
    res.status(200).json({
